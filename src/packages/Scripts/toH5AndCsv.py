...
def WiseToNumpy():
    ...
    for item in payloadWise:
            ...
            if "Accelerometer" in payload:
                ...
                x_matrix.append([x['OAVelocity'], x['Peakmg'], x['RMSmg'], x['Kurtosis'], x['CrestFactor'], x['Skewness'], x['Deviation'], x['Peak-to-Peak Displacement'], time])
                y_matrix.append([y['OAVelocity'], y['Peakmg'], y['RMSmg'], y['Kurtosis'], y['CrestFactor'], y['Skewness'], y['Deviation'], y['Peak-to-Peak Displacement'], time])
                z_matrix.append([z['OAVelocity'], z['Peakmg'], z['RMSmg'], z['Kurtosis'], z['CrestFactor'], z['Skewness'], z['Deviation'], z['Peak-to-Peak Displacement'], time])
...
def WiseToPandas():
    ...
    x_df = pd.DataFrame(x_matrix, columns=["OAVelocity", "Peakmg", "RMSmg", "Kurtosis", "CrestFactor", "Skewness", "Deviation", "Peak-to-Peak Displacement","Time"])
    y_df = pd.DataFrame(y_matrix, columns=["OAVelocity", "Peakmg", "RMSmg", "Kurtosis", "CrestFactor", "Skewness", "Deviation", "Peak-to-Peak Displacement","Time"])
    z_df = pd.DataFrame(z_matrix, columns=["OAVelocity", "Peakmg", "RMSmg", "Kurtosis", "CrestFactor", "Skewness", "Deviation", "Peak-to-Peak Displacement","Time"])
...
def WiseToCsv():
    x_matrix, y_matrix, z_matrix = WiseToPandas()
    columns = ["OAVelocity", "Peakmg", "RMSmg", "Kurtosis", "CrestFactor", "Skewness", "Deviation", "Peak-to-Peak Displacement, Time"]
...
def HexToNumpy():
    ...
    for item in payloadHexParser():
        ...
        if "InletPressure" in payload:
            ...
            output.append([inletPressure, outletPressure, outletTemperature, inverterSpeed,time])
...
def HexToPandas():
    ...
    df = pd.DataFrame(output_matrix, columns=["InletPressure", "OutletPressure", "OutletTemperature", "InverterSpeed", "Time"])
    return df
...
def KSToNumpy():
    ...
    # [{'bn': 'F80332060002BD7B', 'bt': 1676514561}, {'n': 'uplink', 'u': 'count', 'v': 1585}, {'n': 'activation_mode', 'vs': 'ABP'}, {'n': 'datarate', 'vs': 'SF12BW125'}, {'n': 'rssi', 'u': 'dBW', 'v': -90}, {'n': 'snr', 'u': 'dB', 'v': 7.8}, {'n': 'model', 'vs': 'ite11li'}, {'n': 'version', 'vs': '1.0.3.0'}, {'n': 'temperature', 'u': 'Cel', 'v': 52.0}, {'n': 'frequency', 'u': 'Hz', 'v': 60.0}, {'n': 'phaseA_voltage', 'u': 'V', 'v': 221.8}, {'n': 'phaseA_current', 'u': 'A', 'v': 3.5}, {'n': 'phaseA_pwr_factor', 'u': '/', 'v': 0.8600000000000001}, {'n': 'phaseA_active', 'u': 'J', 'v': 1362168000.0}, {'n': 'phaseA_reactive', 'u': 'J', 'v': 195624000.0}, {'n': 'phaseA_tc_config', 'vs': 'POWCT-T16-150-333'}, {'n': 'phaseB_voltage', 'u': 'V', 'v': 221.3}, {'n': 'phaseB_current', 'u': 'A', 'v': 3.6}, {'n': 'phaseB_pwr_factor', 'u': '/', 'v': 0.8799999999999999}, {'n': 'phaseB_active', 'u': 'J', 'v': 1431288000.0}, {'n': 'phaseB_reactive', 'u': 'J', 'v': 159768000.0}, {'n': 'phaseB_tc_config', 'vs': 'POWCT-T16-150-333'}, {'n': 'phaseC_voltage', 'u': 'V', 'v': 221.3}, {'n': 'phaseC_current', 'u': 'A', 'v': 3.55}, {'n': 'phaseC_pwr_factor', 'u': '/', 'v': 0.8700000000000001}, {'n': 'phaseC_active', 'u': 'J', 'v': 1368000000.0}, {'n': 'phaseC_reactive', 'u': 'J', 'v': 121931999.99999999}, {'n': 'phaseC_tc_config', 'vs': 'POWCT-T16-150-333'}, {'n': 'gateway', 'vs': 'F8033202DF790000'}]
        ...
        output.append([temperatura, frequencia, faseA_tensao, faseA_corrente, faseA_fator_potencia, faseA_ativa, faseA_reativa, faseA_config_tc, faseB_tensao, faseB_corrente, faseB_fator_potencia, faseB_ativa, faseB_reativa, faseB_config_tc, faseC_tensao, faseC_corrente, faseC_fator_potencia, faseC_ativa, faseC_reativa, faseC_config_tc, time])
    return output 


def KSToPandas():
    output_matrix = KSToNumpy()
    columns = ["temperature", "frequency", "phaseA_voltage", "phaseA_current", "phaseA_pwr_factor", "phaseA_active", "phaseA_reactive", "phaseA_tc_config", "phaseB_voltage", "phaseB_current", "phaseB_pwr_factor", "phaseB_active", "phaseB_reactive", "phaseB_tc_config", "phaseC_voltage", "phaseC_current", "phaseC_pwr_factor", "phaseC_active", "phaseC_reactive", "phaseC_tc_config","time"]
